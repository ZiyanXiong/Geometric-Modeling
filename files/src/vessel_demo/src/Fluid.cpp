...
{	
	...
	for (int i = 0; i < 100; i++) {
		...
		p->x = p_init + Vector3d::Random().cwiseProduct(Vector3d(0.8, 0.05, 0.05));
	...
	*/
	...
	for (int i = 0; i < 20; i++) {
		...
		// Push twice to add density
		env_particles.push_back(p);
		p->x = (i / 30.0) * cloth->particles[312]->x + (1 - i / 30.0) * cloth->particles[618]->x + Vector3d::Random().cwiseProduct(Vector3d(0.05, 0.05, 0.05)) + Vector3d(0.05, -0.10, 0.05);
	}
	...
	for (int i = 0; i < 20; i++) {
		...
		// Push twice to add density
		env_particles.push_back(p);
		p->x = (i / 30.0) * cloth->particles[412]->x + (1 - i / 30.0) * cloth->particles[564]->x + Vector3d::Random().cwiseProduct(Vector3d(0.05, 0.05, 0.05)) + Vector3d(-0.15, 0.20, 0.05);
	}
...
void Fluid::step(double h, const Eigen::Vector3d& grav, const std::shared_ptr<Cloth> cloth)
{
	...
	float k = 1e-3;			   // Far pressure weight
	float k_near = 1e-2;							// Near pressure weight
	float rest_density = 1.0;

	float k_env = 1e-4;			   // Far pressure weight
	float k_near_env = 1e-1;
	float env_rest_density = 1.0;
	double env_r = 1.0;

	
	if (fmod(t, 0.2) < 0.04 && particles.size() < n - 10) {
		for (int i = 0; i < 20; i++) {
			auto p = make_shared<FluidParticle>();
			particles.push_back(p);
			p->i = i;
			p->r = 0.05;
			//p->m = mass / n;
			p->v = Vector3d(1.0,0.0,0.0);
			//p->x = p_init + Vector3d(unit* (i % 5), unit * (i / 25), unit * ((i / 5) % 5));
			p->x = Vector3d(-1.0, 2.0, 0.0) + Vector3d::Random().cwiseProduct(Vector3d(0.0, 0.05, 0.05)) + Vector3d(0.1, 0.00, 0.00);
			//p->f = Vector3d(0.0, 0.0, 0.0);
		}
	}
	...
	for (auto particle : particles) {
		...
		particle->v += Vector3d(8.8, 0.0, 0.0) * h;
...
void Fluid::draw(std::shared_ptr<MatrixStack> MV, const std::shared_ptr<Program> p, const std::shared_ptr<Shape> sphere, const std::shared_ptr<Shape> cell, const std::shared_ptr<Shape> heart) const
{
	glUniform3fv(p->getUniform("kdFront"), 1, Vector3f(0.53, 0.03, 0.03).data());
	for (auto particle :particles) {
		MV->pushMatrix();
		MV->translate(particle->x.x(), particle->x.y(), particle->x.z());
		MV->rotate(0.5 + (rand() % 100) / 500.0, particle->v.x(), particle->v.y(), particle->v.z());
		MV->scale(0.05);
	...
	glUniform3fv(p->getUniform("kdFront"), 1, Vector3f(0.96, 0.98, 0.97).data());
	for (size_t i = env_particles.size() - 40; i < env_particles.size(); i++) {
		MV->pushMatrix();
	...
	float scale = 0.9;
	if (fmod(t, 0.2) < 0.1) {
		scale -= fmod(t, 0.2) * 0.5;
	}
	else {
		scale = 0.8 + fmod(t, 0.2) * 0.5;
	}
	
	glUniform3fv(p->getUniform("kdFront"), 1, Vector3f(1.0, 0.0, 0.0).data());
	for (size_t i = env_particles.size() - 40; i < env_particles.size(); i++) {
		MV->pushMatrix();
		MV->translate(-1.8, 1.0, 1.5);
		MV->scale(scale);
		glUniformMatrix4fv(p->getUniform("MV"), 1, GL_FALSE, glm::value_ptr(MV->topMatrix()));
		heart->draw(p);
		MV->popMatrix();
