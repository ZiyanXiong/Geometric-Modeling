...
void Scene::load(const string &RESOURCE_DIR)
{
	...
	double mass = 0.1;
	double stiffness = 1;
	Vector3d x00(0.0, 3.0, 0.0);
	Vector3d x01(1.0, 0.0, 0.0);
	Vector3d x10(-0.75, 0.5, 0.2);
	Vector3d x11(0.75, 0.5, 0.2);
	cloth = make_shared<Cloth>(rows, cols, x00, x01, x10, x11, mass, stiffness);
	fluid = make_shared<Fluid>(500, x00, Vector3d (0.0, 0.0, 0.0), cloth);
