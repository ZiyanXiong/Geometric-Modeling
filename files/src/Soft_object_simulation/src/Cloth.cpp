			 ...
			 double stiffness)
{
	...
	// Create particles
	n = 0;
	double r = 0.005; // Used for collisions
	int nVerts = rows * cols + 2;

	int sectorCount = cols;
	int stackCount = rows;
	float radius = x01.norm();
	...
	float stackStep = PI / (stackCount + 1);
	...
	for (int i = 0; i < 2; i++) {
		...
		particles.push_back(p);
		p->r = r;
		p->x = radius * Vector3d(0.0, sinf(PI/2 - i* PI), 0.0);
		p->x += x00;
		p->v << 0.0, 0.0, 0.0;
		p->m = mass / (nVerts);
		p->fixed = false;
	...
	*/
	...
	for(int i = 0; i < rows; ++i) {
		...
		int k1 = (i + 1) * cols - 1;
		springs.push_back(createSpring(particles[k0], particles[k1], stiffness));
		springs.push_back(createSpring(particles[k0 - 1], particles[k1 - 1], stiffness));
	...
	// Create polar springs
	for (int j = 0; j < cols; j++) {
		int k_0 = cols * rows;
		int k_1 = k_0 + 1;
		for (int i = 0; i < 5; i++) {
			double softer_stiffness = stiffness;
			springs.push_back(createSpring(particles[k_0], particles[i * cols + j], softer_stiffness));
			springs.push_back(createSpring(particles[k_1], particles[(rows - i) * cols - j - 1], softer_stiffness));
		}
	}
	...
	for(int i = 0; i < rows-1; ++i) {
		...
		eleBuf.push_back(i* cols);
		eleBuf.push_back((i + 1)* cols);
	}
	
	for (int i = 0; i < 2; i++) {
		int index = cols * rows + i;
		eleBuf.push_back(index);
		for (int j = 0; j < cols; j++) {
			eleBuf.push_back(j + i*(index - cols - 1));
		}
		eleBuf.push_back(i* (index - cols - 1));
	}
...
void Cloth::updatePosNor()
{
	...
	for(int i = 1; i < rows - 1; i += 2) {
		for(int j = 1; j < cols - 1; j ++) {
			int k = i*cols + j;
			subPos.push_back(0.66 * particles[k]->x + 0.33 * particles[k - 1]->x);
			subPos.push_back(0.66 * particles[k]->x + 0.33 * particles[k + 1]->x);
			subPos.push_back(0.66 * particles[k]->x + 0.33 * particles[k - cols]->x);
			subPos.push_back(0.66 * particles[k]->x + 0.33 * particles[k - cols + 1]->x);
			subPos.push_back(0.66 * particles[k]->x + 0.33 * particles[k - cols - 1]->x);
			subPos.push_back(0.66 * particles[k]->x + 0.33 * particles[k + cols]->x);
			subPos.push_back(0.66 * particles[k]->x + 0.33 * particles[k + cols + 1]->x);
			subPos.push_back(0.66 * particles[k]->x + 0.33 * particles[k + cols - 1]->x);
	...
	for (int i = 0; i < 2; i++) {
		...
		Vector3d nor(0.0, 0.0, 0.0);
		for (int j = 0; j < cols - 1; j++) {
			nor += ((particles[j + i * (rows - 1) * cols]->x - particles[top]->x).cross(particles[j + 1 + i * (rows - 1) * cols]->x - particles[top]->x)).normalized();
		}
		nor /= cols - 1;
		if (i == 0) {
			nor = -nor;
		...
		norBuf[3 * top] = nor(0);
		norBuf[3 * top + 1] = nor(1);
		norBuf[3 * top + 2] = nor(2);
	}
}
	...
	for (auto particle : particles) {
		if (particle->i > 0 && particle->x.y() >0) {
			for (auto sphere : fluid->particles) {
				...
				double d = 1.0 +  sphere->r - delta_x.norm();
...
void Cloth::draw(shared_ptr<MatrixStack> MV, const shared_ptr<Program> p) const
{
	...
	for(int i = 0; i < rows - 1; ++i) {
		glDrawElements(GL_TRIANGLE_STRIP, 2 * (cols + 1), GL_UNSIGNED_INT, (const void *)(2 * (cols + 1) * i*sizeof(unsigned int)));
	}
	
	//glDrawElements(GL_TRIANGLE_FAN, 1, GL_UNSIGNED_INT, (const void*)(2 * (cols + 1) * rows * sizeof(unsigned int)));
	
	for (int i = 0; i < 2; i++) {
		glDrawElements(GL_TRIANGLE_FAN, cols + 2, GL_UNSIGNED_INT, (const void*)((2 * (cols + 1) * (rows - 1) + i * (cols + 2))* sizeof(unsigned int)));
	}
